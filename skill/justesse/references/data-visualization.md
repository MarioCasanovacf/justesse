...
- In a scatter, relationship, or distribution plot, name the salient points directly: the extremes,
  the outliers, and any exemplar the argument rests on. A cloud of anonymous marks hides the very
  observations that carry the finding. Offset each label from its mark and keep it legible over dense
  regions with a background-coloured halo or a short leader; ring or otherwise emphasize the point it
  belongs to. Do not label every point; choose the few the reader must be able to name.
- When marks are grouped by a categorical hue, keep a colour legend even where salient points are
  named: the labels identify individuals, the legend decodes the groups, and neither substitutes for
  the other. Repeat the legend in every view that relies on the encoding rather than assuming the
  reader carries it over from an earlier one.
...
Without a profile, derive color from the product or supplied brand rather than importing a palette
from anywhere else. Under `personal:<name>`, apply the declared semantic values and the chart
recipes in `profile-data-semantics.md`.
